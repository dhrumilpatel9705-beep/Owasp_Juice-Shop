...
export function login() {
  function afterLogin(user: User, res: Response, next: NextFunction) {
    ...
    const visitorId = requestContextStore.getStore()?.visitorId
    if (visitorId && user.id) {
      challengeUtils.mergeProgress(visitorId, user.id).catch((err: Error) => {
        console.error('Failed to merge visitor progress on login:', err)
      })
    }
