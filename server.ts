...
function configureApp(app: ReturnType<typeof express>, seq: typeof sequelize) {
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    loadContext().then(() => {
      requestContextStore.run({ userId, visitorId, solvedChallenges, codingChallengeStatuses }, () => {
        next()
      })
    }).catch((err) => {
      logger.error('Error loading request context: ' + utils.getErrorMessage(err))
      next(err)
    })
  })
  ...
  app.get('/api/Challenges', async (req: Request, res: Response) => {
    try {
      const allChallenges = await ChallengeModel.findAll({
        include: [ChallengeDependencyModel]
      })
      ...
      let userSolvedChallenges: UserChallengeModel[] = []
      if (userId || visitorId) {
        const whereClause: any = userId ? { UserId: userId } : { visitorId }
        userSolvedChallenges = await UserChallengeModel.findAll({ where: whereClause })
      }

      const solvedMap = new Map<string, { solved: boolean, codingChallengeStatus: number }>()
      userSolvedChallenges.forEach((uc) => {
        solvedMap.set(uc.challengeKey, {
          solved: uc.solved,
          codingChallengeStatus: uc.codingChallengeStatus
        })
      })

      const responseData = allChallenges.map((challenge) => {
        const userStatus = solvedMap.get(challenge.key)
        const plainChallenge = challenge.get({ plain: true })
        plainChallenge.solved = userStatus ? userStatus.solved : false
        plainChallenge.codingChallengeStatus = userStatus ? userStatus.codingChallengeStatus : 0
        return plainChallenge
      })

      res.json({ status: 'success', data: responseData })
    } catch (err) {
      res.status(500).json({ status: 'error', error: utils.getErrorMessage(err) })
    }
  })
